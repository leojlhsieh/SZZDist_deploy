...
def train(config=None):
    ...
    with wandb.init(config=config, name=wanb_name):
        ...
        print("image_transform = build_transforms(config['bpm_color'], device=device)")
        image_transform = build_transforms(config['bpm_color'], device=device)

        print("tarin_loader, test_loader, data_info = build_dataset(config['data_name'], config['batch_size'])")
        tarin_loader, test_loader, data_info = build_dataset(config['data_name'], config['batch_size'])
        
        print("model_bpm, model_feature, model_classifier = build_model(config['bpm_color'], config['bpm_mode'], config['bpm_depth'], config['bpm_width'], config['bpm_parallel'], config['model_feature'], device=device)")
        ...
        print("pprint(data_info, sort_dicts=False)")
