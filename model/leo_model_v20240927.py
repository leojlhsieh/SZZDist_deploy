...
# reserve modulation for training parameters
modulation_indices.append(z2index(Ldist))
modulation_indices.append(z2index(Ldist*3))
modulation_indices.append(z2index(Ldist*5))
modulation_indices.append(z2index(Ldist*7))
modulations.append(torch.ones_like(gaussian_beam_bpm))  # reserve for training parameters
modulations.append(torch.ones_like(gaussian_beam_bpm))  # reserve for training parameters
modulations.append(torch.ones_like(gaussian_beam_bpm))  # reserve for training parameters
modulations.append(torch.ones_like(gaussian_beam_bpm))  # reserve for training parameters

'''
# additional modulation, e.g mirrow angle, lens, etc.
modulation_indices.append(z2index(Ldist*2))
modulations.append(blazed_grating_bpm)

modulation_indices.append(z2index(Ldist*4))  # This two show cancel out. Check in one index can have multiple modulations
modulation_indices.append(z2index(Ldist*4))
modulations.append(blazed_grating_bpm)
modulations.append(blazed_grating_bpm.conj())

modulation_indices.append(z2index(Ldist*6))
modulations.append(lens_bpm)
'''


modulation_indices = torch.tensor(modulation_indices)
modulations = torch.stack(modulations)  # use stack beacuse modulations is a list of tensors



if VERBOSE:
    print(f"{modulation_indices.shape=}, {modulations.shape=}")
    print(f"{modulation_indices.dtype=}, {modulations.dtype=}")
    print(f"{modulation_indices=}")
...
class SZZBPM(nn.Module):
    ...
    def __init__(self, batch_size=1, device=None, **kwargs):
        super().__init__()
        ...
        # self.keep_propagation = keep_propagation.detach().clone().to(device)
        # self.Nz_bpm = Nz_bpm.detach().clone().to(device)
        if self.keep_propagation:
            batch, channel, height, width = self.field_in.shape
            self.propagation = torch.empty((batch, channel, height, width, self.Nz_bpm), dtype=self.field_in.dtype, device=self.field_in.device)
            propagation[:, :, :, :, 0] = self.field_in
        # self.modulation_indices = modulation_indices.detach().clone().to(device)
        # self.modulations = modulations.detach().clone().to(device)
        self.modulations = torch.cat([self.modulations for _ in range(batch_size)], dim=1)  # index, batch, channel, height, width
        ...
        self.phase_scale4 = Parameter(torch.randn((1, 1, Ny_slm, Nx_slm), dtype=torch.float32, device=device))
        ...
        self.phase_bias4 = Parameter(torch.randn((1, 1, Ny_slm, Nx_slm), dtype=torch.float32, device=device))
        # SLM and BPM coordinate conversion
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        # modulation1_bpm = torch.exp(1j*modulation1_phase_bpm)
        # self.modulations[0] = modulation1_bpm
        ## combine in oneline
        self.modulations[0] = torch.exp(1j*(input * (self.field_interpolate(input * self.phase_scale1 + self.phase_bias1, size=self.ny_nx_slm2bpm, pad=self.pad_slm2bpm))))
        ...
        # modulation2_bpm = torch.exp(1j*modulation2_phase_bpm)
        # self.modulations[1] = modulation2_bpm
        self.modulations[1] = torch.exp(1j*(input * (self.field_interpolate(input * self.phase_scale2 + self.phase_bias2, size=self.ny_nx_slm2bpm, pad=self.pad_slm2bpm))))
        ...
        # modulation3_bpm = torch.exp(1j*modulation3_phase_bpm)
        # self.modulations[2] = modulation3_bpm
        self.modulations[2] = torch.exp(1j*(input * (self.field_interpolate(input * self.phase_scale3 + self.phase_bias3, size=self.ny_nx_slm2bpm, pad=self.pad_slm2bpm))))

        # modulation4_phase_slm = input * self.phase_scale4 + self.phase_bias4
        # modulation4_phase_bpm = self.field_interpolate(modulation4_phase_slm, size=self.ny_nx_slm2bpm, pad=self.pad_slm2bpm)
        # modulation4_bpm = torch.exp(1j*modulation4_phase_bpm)
        # self.modulations[3] = modulation4_bpm
        self.modulations[3] = torch.exp(1j*(input * (self.field_interpolate(input * self.phase_scale4 + self.phase_bias4, size=self.ny_nx_slm2bpm, pad=self.pad_slm2bpm))))

        # BPM core
        u0 = self.field_in
        for z_index in range(1, Nz_bpm):
            temp_indices = [i for i, x in enumerate(self.modulation_indices) if x == z_index]
            for i in temp_indices:
                u0 = u0 * self.modulations[i]
            u1 = torch.fft.ifft2(torch.fft.fft2(u0) * self.DFR) * self.super_gaussian
            u0 = u1
            if self.keep_propagation:
                self.propagation[:, :, :, :, z_index] = u1
        field_out = u1
