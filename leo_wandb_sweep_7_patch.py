...
def called_by_wandb_sweep_agent():
    with wandb.init(config=vars(args), name=args.wanb_runname):  # sweep agent will overwrite if already exists in args
        ...
        # }
        if config.loss_ratio == 1.0 or config.lr_bpm < 0.0015 or config.lr_class < 0.00009:
            # Early end. Because these does not worth running.
            # Remove loss_ratio = 1.0, because the final classifier need to be trained.
            # Remove the smallest 2 option of lr_bpm, because current result shows that larger lr_bpm is better.
            # Remove the smallest 2 option of lr_class, because current result shows that larger lr_class is better.
            logging.info("This run is not worth running. Early end.")
            wandb.log({"test/accuracy": -1}) # A obvious value to indicate that this run is not worth running
            return
        ...
        for epoch in range(1, config.epochs+1):  # count epochs starting from 1
            ...
            for batch, data in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Epoch {epoch:02d} train", unit="batch", disable=(not config.verbose)):
            ...
            with torch.inference_mode():
                # ===== Testing batch loop =====
                for batch, data in tqdm(enumerate(test_loader), total=len(test_loader), desc=f"Epoch {epoch:02d} test", unit="batch", disable=(not config.verbose)):
                    image = data['image']  # Already in device
                    feature = data['feature_finetune_vit']
                    label = data['label']
