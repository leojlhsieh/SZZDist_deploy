...
Lz_bpm = 6*2*5e-3  # propagation distances [m]
...
mod_idx.append(z2index(Ldist*8))
mod_08 = torch.ones_like(field_in) # Angle of mirror  # Dim should be 4, [Batch, Channel, Height, Width]

mod_idx.append(z2index(Ldist*9))
# mod_09 = SLM4  # Dim should be 4, [Batch, Channel, Height, Width]
...
class SZZBPM(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        mod_09 = torch.exp(1j*modulation4_phase_bpm)
        ...
        u0 = u0 * self.mod_08
        u0 = self.freespace_bpm(field=u0, Nsteps=self.mod_spacing[8])

        u0 = u0 * mod_09  # weight * SLM + bias
        u0 = self.freespace_bpm(field=u0, Nsteps=self.mod_spacing[9])
